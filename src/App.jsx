...
function drawdownsFromIndex(idxMap) {
  let maxSF = -Infinity; const out = []; Object.keys(idxMap).forEach(d => {
  ...
  for (const c of cols) {
    ...
    let i = 0; while (i < a.length) {
      if (a[i] == null) {
        ...
        if (i > 0 && j < a.length) {
          const v0 = a[i - 1], v1 = a[j]; const steps = j - i + 1;
...
function encodeSavedPortfoliosToUrl(savedPortfolios) {
  try {
    // Stringify and handle Unicode for base64 encoding
    const jsonString = JSON.stringify(savedPortfolios);
    // encodeURIComponent escapes all characters except: A-Z a-z 0-9 - _ . ! ~ * ' ( )
    // unescape converts %XX hex sequences into ASCII characters, allowing btoa to work
    // accessible way to handle utf-8 in btoa
    const encoded = btoa(unescape(encodeURIComponent(jsonString)));
    return encoded;
  } catch (error) {
    console.error('Error encoding saved portfolios:', error);
    return null;
  }
}

function decodeSavedPortfoliosFromUrl(encodedString) {
  try {
    // Reverse the encoding process: atob -> escape -> decodeURIComponent -> parse
    const jsonString = decodeURIComponent(escape(atob(encodedString)));
    const portfolios = JSON.parse(jsonString);
    return Array.isArray(portfolios) ? portfolios : null;
  } catch (error) {
    console.error('Error decoding saved portfolios:', error);
    return null;
  }
}

function getAllPortfoliosShareUrl(savedPortfolios) {
  const encoded = encodeSavedPortfoliosToUrl(savedPortfolios);
  if (!encoded) return null;

  const baseUrl = window.location.origin + window.location.pathname;
  return `${baseUrl}?portfolios=${encoded}`;
}
...
export default function App() {
  ...
  const [showToast, setShowToast] = useState(false); // Added for toast message
  ...
  useEffect(() => {
    if (!isLoadingData && rows.length > 0) {
      ...
      const portfoliosParam = urlParams.get('portfolios');
      if (portfoliosParam) {
        const importedPortfolios = decodeSavedPortfoliosFromUrl(portfoliosParam);
        if (importedPortfolios && importedPortfolios.length > 0) {
          // Merge imported portfolios with existing ones, avoiding duplicates by ID or exact content
          setSavedPortfolios(prev => {
            const currentIds = new Set(prev.map(p => p.id));
            const newPortfolios = importedPortfolios.filter(p => !currentIds.has(p.id));

            // If IDs clash (unlikely unless re-importing same), we could regenerate IDs or just skip.
            // Let's assume user wants to add them. We should probably regenerate IDs to be safe if they clash.
            // But if they are exactly the same content, maybe skip?
            // For simplicity, let's just add ones that don't share an ID.

            // Actually, let's regenerate IDs for imported ones to avoid conflicts and treating them as "new" copies
            const secureImported = newPortfolios.map(p => ({
              ...p,
              id: Date.now().toString() + Math.random().toString(36).substr(2, 5), // regenerate ID
              name: p.name + " (Imported)"
            }));

            const combined = [...prev, ...secureImported];
            savePortfoliosToStorage(combined);
            return combined;
          });
          setShowPortfolioManager(true);
          setShowToast(true); // Re-use toast to indicate success
          setTimeout(() => setShowToast(false), 2000);
        }
      }

      // Cleanup URL if any param was processed
      if (portfolioParam || portfoliosParam) {
        const newUrl = window.location.origin + window.location.pathname;
        window.history.replaceState({}, document.title, newUrl);
      }
    }
  ...
  const handleFile = async (e) => {
    ...
    if (f.name.endsWith(".xlsx") || f.name.endsWith(".xls")) {
      ...
      const cols = header.split(","); normalized = lines.map(line => {
        const parts = line.split(","); const row = {};
  ...
  const handleShareAllPortfolios = async () => {
    const url = getAllPortfoliosShareUrl(savedPortfolios);
    if (url) {
      try {
        await copyToClipboard(url);
        setShowToast(true);
        ...
        console.error('Failed to copy URL:', error);
        alert("URL might be too long to copy/share directly. Try sharing fewer portfolios.");
      }
    }
  };
              ...
              {savedPortfolios.length > 0 && (
                <div className="border border-bloomberg-border p-4 rounded">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-medium text-xs uppercase tracking-wide text-bloomberg-text-dim">Saved Portfolios ({savedPortfolios.length})</h3>
                    <button
                      onClick={handleShareAllPortfolios}
                      className="px-2 py-1 bg-bloomberg-panel border border-bloomberg-border text-bloomberg-text text-xs hover:bg-bloomberg-border rounded transition-colors"
                      title="Share link for all saved portfolios"
                    >
                      SHARE ALL
                    </button>
                  </div>
              ...
              {columns.map((c) => (
                <li key={c} className="py-2 flex items-center justify-between">
                  <span className="text-xs flex items-center gap-2">
                    <span className={`text-[9px] px-2 py-0.5 border capitalize text-center w-20 shrink-0 font-mono rounded ${getAssetCategory(c) === "stocks" ? "bg-bloomberg-bg border-bloomberg-accent text-bloomberg-accent" :
